...
The universal naming convention of the wiki. Every page ID follows the pattern `{type}.{slug}` — a concatenation of the page type prefix and a human-readable, URL-safe slug derived from the title. Examples: `entity.seneca`, `concept.praemeditatio-malorum`, `source.schneider-metastudy-2024`. Page IDs are unique vault-wide. Claims are referenced via the combined notation `{page-id}#{claim-slug}`, e.g. `entity.seneca#claim-cortisol-senkung`. Slugs are always lowercase, hyphenated, and stay stable once created. This convention makes all content machine-indexable and human-scannable without a database.
